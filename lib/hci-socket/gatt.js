...
Gatt.prototype.onAclStreamData = function(cid, data) {
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    this._currentCommand.completed = true;
    this._currentCommand.callback(data);

    debug(this._address+": Command Done seq:"+this._currentCommand.sequence+" qpos:"+this._currentCommand.possition+" qlen:"+this._commandQueue.length);
...
Gatt.prototype._doNextCommand = function() {
    this._currentCommand = null;
    while(this._commandQueue.length) {
      ...
      if (this._currentCommand.callback) {
        this._readTimeout(this._currentCommand);
        break;
...
Gatt.prototype._readTimeout = function(command) {
    var that = this;
    // if the read command
    command.retries = command.retries || 0;
    setTimeout(() => {
      if ( !command.completed ) {
        debug(this._address+": Command timedout seq:"+command.sequence+" retries:"+command.retries);
        that.writeAtt(command.buffer);
        command.retries++;
        if ( command.retries > 3 ) {
          debug(this._address+": Command failed seq:"+command.sequence+" retries:"+command.retries);
...
Gatt.prototype._queueCommand = function(buffer, callback, writeCallback) {
  debug(this._address+": Adding Command seq:"+this._currentCommand.sequence+" qpos:"+this._currentCommand.possition+" qlen:"+this._commandQueue.length+" buffer:"+buffer.toString("hex"));
  this.commandSequence++;
  this._commandQueue.push({
    buffer: buffer,
    sequence: this.commandSequence,
    possition: this._commandQueue.length,
    callback: callback,
